...
Deterimin max in far right column (values in a list)
Backtrack up to Row 0.  No penalty for before after
 At runtime, pick from a list of max values.  

'''
###   Multiple scoring methods can be called
##  For Fitting, full string of w, substring of v, find max in bottom row
###
def MaxBottom(npG,TopWord):
    ...
    return Max, ix, numCols, Startpoints
...
def main():
    ...
    big, iiy, iix, StartList=MaxBottom(npGrid,AncString) 
    ...
    p=int(pick)
    iiy=StartList[p]
    print("Using ", iiy," as a start point.") 
    i=iiy
    j=iix
    #i+=1
    #j+=1
